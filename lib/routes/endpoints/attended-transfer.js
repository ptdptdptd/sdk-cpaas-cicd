 ...
 */
const gatherTN = {
  actionHook: '/attended-transfer/collect',
  input: ['digits'],
  finishOnKey: '*',
  numDigits: 11,
  timeout: 20,
...
router.post('/', (req, res) => {
  ...
  logger.debug({payload: req.body, query: req.query}, 'POST /attended-transfer');
...
router.post('/dtmf', async(req, res) => {
  ...
  logger.debug({payload: req.body}, 'POST /attended-transfer/dtmf');
  const dtmf = req.body.dtmf;
  let obj ;
  if ('*2' === dtmf) {
    obj = {
      child_call_hook: `/attended-transfer/park?parkingSlot=${req.body.call_sid}`,
      call_hook: '/attended-transfer'
    };
  }
  else if ('*3' === dtmf) {
    obj = {
      child_call_hook: `/attended-transfer/complete-transfer?parkingSlot=${req.body.call_sid}`
    };
  }
...
router.post('/park', async(req, res) => {
  const {logger} = req.app.locals;
  logger.debug({payload: req.body, query: req.query}, 'POST /attended-transfer/park');
